    ...
    if (mGet.length > 0) {
        ...
        for (let i = 0; i < mGet.length; i++) {
            ...
            var InputComponent = /*html*/`<input
                            ...
                            class='form-input-style-papper shadow-sm ${$(
                              mGet[i]
                            ).attr('name')}'
                            mode="input"
    ...
    if (autoGet.length > 0) {
        for (let i = 0; i < autoGet.length; i++) {
            if (
                ...
                if(data.length > 0){
                    var resultset = data.find(
                        (o) => o?.nama_jabatan == $($(autoGet[i])).attr('fildquery')
                    );
                    console.log("_",resultset);
                    var value = !__empty(resultset?.alias)?resultset?.alias:resultset?.nama_jabatan;
                    if (resultset != undefined && resultset != null) {
                        source =
                            $(source).find(
                                `font[type='auto'][name='nama_jabatan'][fildquery='${$($(autoGet[i]))
                                    .attr('fildquery')}']`
                            )
                            .text(value)
                            .css({
                    ...
                    $(source)
                    .find(
                        `font[type='auto'][name='nama_jabatan'][fildquery='${$($(autoGet[i])).attr('fildquery')}']`
                    )
